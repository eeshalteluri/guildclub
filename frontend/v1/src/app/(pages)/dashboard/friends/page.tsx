...
useEffect(() => {
  const fetchFriends = async() => {
    ...
    try{
    const response = await fetch(`https://guildclub-backend.vercel.app/friend?username=${user?.username}`, {
      method: "GET",  
      credentials: "include",
    })
...
useEffect(() => {
    const fetchRequests = async function getRequests() {
      ...
      try {    
        const response = await fetch(`https://guildclub-backend.vercel.app/request`, {
            credentials: 'include',
            method: 'GET',
          });
