...
const TaskCard: React.FC<TaskCardProps> = (taskData) => {
  ...
  const handleTodayLog = async () => {
    console.log("handle today's task initialized.");
    try {
        const response = await fetch(`/api/start-agenda`, {
            method: "POST"
        });
  ...
  const todayTempLogColor = getStatusColor(todayTempLogStatus)
    ...
    <div className="flex items-start">
      ...
      <p className="ml-2 text-sm">
        ...
        <div className="flex justify-center items-center">
          <Button onClick={handleTodayLog} disabled={!!!doesTodayExist}>{doesTodayExist?.status === "pending" ? "Mark": doesTodayExist?.status === "approval" ? "Send Approval": doesTodayExist?.status === "completed" ? "Unmark": "Mark"}</Button>
          <Popover>
            <PopoverTrigger asChild>
              <EllipsisVertical />
            </PopoverTrigger>
